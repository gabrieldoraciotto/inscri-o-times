...
export default function App() {
  ...
  function validate() {
    ...
    form.members.forEach((m, i) => {
      if (!m.trim()) e[`member${i}`] = 'Nome obrigatório.'
    })
    return e
  }
  ...
  async function handleSubmit() {
    ...
    const duplicate = teams.find(
      t => t.team_name.toLowerCase() === form.teamName.trim().toLowerCase()
    )
    if (duplicate) {
      setErrors({ teamName: 'Já existe um time com esse nome.' })
      setSaving(false)
      return
    }
    ...
    <div className="container">
      ...
      <div className="tabs">
        ...
        <button
          ...
          onClick={() => { setView('teams'); loadTeams() }}
        >
      ...
      {view === 'teams' && (
        <div>
          {loading ? (
            <p className="empty">Carregando times...</p>
          ) : teams.length === 0 ? (
      ...
      <p className="footer-note">
        Inscrições abertas ao público — dados compartilhados entre todos.
      </p>
    </div>
  )
}
